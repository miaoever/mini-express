...
var _merge = function (base, proto) {
  for (key in proto) {
    if (proto.hasOwnProperty(key)) {
      base[key] = proto[key].bind(base);
    }
  }
}

var _done = function (err, req, res) {
  if (err) {
    res.statusCode = 500;
  } else {
    res.statusCode = 404;
...
var _httpVerbGenerator = function(app) {
...
app.handle = function(req, res, done) {
  ...
  var next = function(err) {
    ...
    req.url = req.url.substr(prefix.length);//add subRoute for subApp
...
module.exports = function () {
  _httpVerbGenerator(app);
  var factory = function(req, res) {
    factory.handle(req, res, _done); 
  }
  _merge(factory, app);
  factory.stack = [];
  return factory;
